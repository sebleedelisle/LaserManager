...
//		}
		...
		for(int i = startshapes; i<endshapes; i++) {
			...
			scale*=scale;
			scale*=1.5;
			lm.addLaserRectEased(centre + (ofPoint(-400,-480)*scale), ofPoint(800,600,0)*scale, ofColor::white);
	...
	// these bits of code are like particle renderers for lines and circle objects
